...
int main(int argc, char **argv)
{
    ...
    Mmu mmu = Mmu(67108864, page_size, memory); // Create MMU
    ...
    while (command != "exit") {
        ...
        else if (strcmp(token, "allocate")==0)
        {
            ...
            //allocate extra pages for large variables
            while (temp_size > page_size)
            {
                pagetable.addEntry(pid, mmu.getProcess(pid)->num_pages);
                mmu.getProcess(pid)->num_pages++;
                temp_size -= page_size;
            }
        ...
        else if (strcmp(token, "free")==0)
        {
            ...
            pagetable.deletePagesOfVar(pid, var);
